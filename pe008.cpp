...
int main() {
  long greatest = 0;
  for (int x = 0; x < (maxdigits - (adjacent - 1)); ++x) {
    int subarray[adjacent];
    for (int y = 0; y < adjacent; ++y) {
      subarray[y] = digits[x + y];
    }

    long product = product(subarray);
    if (product > greatest) {
      greatest = product;
    }
  }
